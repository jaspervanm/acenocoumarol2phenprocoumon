...
We analysed the prospective registration of INRs and dosing proposals to determine the effect of a switch from acenocoumarol to phenprocoumon on quality of anticoagulation parameters. We hypothesised that this switch would increase TTR and decrease INR variability.
We compared switchers to non-switchers to account for the natural evolution of TTR and INR variability over time.

## Data and patient selection

We extracted INR records of adult patients who used acenocoumarol or switched from acenocoumarol to phenprocoumon, in target ranges 2–3, 2.5–3.5 or 2–3.5 (used for low-intensity anticoagulation before switching to the internationally recommended target range of 2–3 on January 1st, 2016), indicated for atrial fibrillation (AF), venous thromboembolism (VTE) or mechanical heart valve replacement (MVR).
...
We distinguished three periods: the 180 days before a patient's switch to phenprocoumon, the 180 days after switch, and a period from 180 to 360 days (see Figure 1).
...
**_Figure 1. Periods distinguished in the analyses._**
...
```
## Patient selection
Patient selection is outlined in Figure 2.  
...
There are relatively more switchers in Maastricht than in Groningen.
...
**_Figure 2. Flowchart of patient selection in this study._**
...
## Effect of switching
The evolution of VKA quality parameters after switching from acenocoumarol to phenprocoumon is summarised in Table 2.
...
```{r child = "tables/switchers_qoac.md"}
```

Table 2. VKA quality parameters in patients who switched from acenocoumarol to phenprocoumon.  
...
The difference in outcomes between switchers and matched non-switchers is shown in Table 3 and Figure 3.
...
```{r child="tables/matched_overall.md"}
```

Table 3. Difference in changes since baseline of switchers versus matched non-switching controls, with 95% confidence interval.  
An asterisk indicates that values were log-transformed in the analyses.  
P-values from a linear mixed model.


**_Figure 3. Proportion of time spent in, above or below the target range in switchers and matched non-switchers, alongside INR variability in the same groups._**    
...
The effect of a switch relative to non-switchers is summarised in Table S5 and shown in Figure 4.

**_Figure 4. Changes in time in therapeutic range (TTR) and international normalised ratio (INR) variability in patients who switched from acenocoumarol to phenprocoumon, relative to matched non-switchers._**  
*Not all subgroups could be formed in all target ranges; this could cause missing values.*  
